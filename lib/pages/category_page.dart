...
class _CategoryPageState extends State<CategoryPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar:
            AppBar(title: Text('商品分类', style: TextStyle(color: Colors.white))),
        body: Container(
          child: Row(
            children: <Widget>[
              LeftCategoryNav(),
  ...
  void _getCategory() async {
    await request('getCategory').then((val) {
      ...
      Provide.value<ChildCategory>(context)
          .getChildCategory(mlist[0].bxMallSubDto);

      _getGoodList(
          categoryId: mlist[0].mallCategoryId,
          categorySubId: ""); //先拉取第一个大类，第二个小类的全部商品列表
    });
  }
      ...
      onTap: () {
        setState(() {
          ...
          if (listIndex != index) {
            ...
            _getGoodList(
                categoryId: mCategoryId, categorySubId: mCategorySubId);
          }
          listIndex = index;
        ...
        Provide.value<ChildCategory>(context)
            .getChildCategory(mChildList); //状态管理给二级分类集合赋值
      },
      child: Container(
...
class _RightCategoryNavState extends State<RightCategoryNav> {
  ...
  Widget _rightInkWell(BxMallSubDto item, int index) {
    ...
    return InkWell(
      ...
      child: Container(
        decoration:
            BoxDecoration(color: isClick ? Colors.pink[50] : Colors.white),
        height: ScreenUtil().setHeight(100),
